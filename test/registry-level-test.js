...
describe('Proof Of Humanity Level Contract', function() {
    ...
    describe('Change Governor', function() {
        ...
        beforeEach(async function(){
            ...
            proofOfHumanityLevelContract = await contractFactory.deploy(pohContract.address, pohContract.address);
            await proofOfHumanityLevelContract.deployed()
        });

        it('should fail to change the contract governor', async function() {
            await expect(proofOfHumanityLevelContract.connect(userOne).changeGovernor(userOne.address))
        ...
        it('should change the contract governor ok', async function() {
            ...
            await expect(tx).to.emit(proofOfHumanityLevelContract, 'GovernorChanged').withArgs(userOne.address);
        });
    ...
    describe('Change Proof Of Humanity', function() {
        ...
        beforeEach(async function(){
            ...
            proofOfHumanityLevelContract = await contractFactory.deploy(firstPoHContract.address, secondPoHContract.address);
            await proofOfHumanityLevelContract.deployed()
        });

        it('ProofOfHumanity core should change to the third contract', async function() {
            const tx = await proofOfHumanityLevelContract.changeProofOfHumanityCore(thirdPohContract.address);
            const proofOfHumanityCore = await proofOfHumanityLevelContract.proofOfHumanityCore();
            
            await expect(tx).to.emit(proofOfHumanityLevelContract, 'ProofOfHumanityCoreChanged').withArgs(thirdPohContract.address);
        ...
        it('ProofOfHumanity soft should change to the third contract', async function() {
            const tx = await proofOfHumanityLevelContract.changeProofOfHumanitySoft(thirdPohContract.address);
            const proofOfHumanitySoft = await proofOfHumanityLevelContract.proofOfHumanitySoft();
            
            await expect(tx).to.emit(proofOfHumanityLevelContract, 'ProofOfHumanitySoftChanged').withArgs(thirdPohContract.address);
        ...
        it('should fail becase the sender is not the governor change to the third contract', async function() {
            await expect(proofOfHumanityLevelContract.connect(userOne).changeProofOfHumanityCore(thirdPohContract.address))
                .to.be.revertedWith(ERRORS.ONLY_GOVERNOR_TRANSACTION);
                await expect(proofOfHumanityLevelContract.connect(userOne).changeProofOfHumanitySoft(thirdPohContract.address))
                .to.be.revertedWith(ERRORS.ONLY_GOVERNOR_TRANSACTION);
        });
    });
    ...
    describe('Act as Proxy with a valid Proof Of Humanity contract', function() {
        ...
        beforeEach(async function(){
            ...
            proofOfHumanityLevelContract = await contractFactory.deploy(pohCoreContract.address, pohSoftContract.address);
            await proofOfHumanityLevelContract.deployed()
        });
        ...
        it('should be the same registry asking through the proxy', async function() {
            ...
            expect(await proofOfHumanityLevelContract.isRegistered(userTwo.address)).to.equal(false);
        });
        ...
        it('should be the another registry if there is a changeProofOfHumanity transaction first', async function() {
            ...
            await proofOfHumanityLevelContract.changeProofOfHumanityCore(anotherPoHCoreContract.address);
            expect(await pohCoreContract.isRegistered(userOne.address)).to.equal(true);
            expect(await anotherPoHCoreContract.isRegistered(userOne.address)).to.equal(false);
            expect(await proofOfHumanityLevelContract.isRegistered(userOne.address)).to.equal(false);
        });
    });
    ...
    describe('Act as proxy with submissions on soft contract', function() {
        ...
        it('isSoftRegistered should return true if its in both registries', async function() {
            await pohSoftContract.connect(userOne).addSubmission('evidence', 'name');
            await pohCoreContract.connect(userOne).addSubmission('evidence', 'name');
            expect(await proofOfHumanityLevelContract.isSoftRegistered(userOne.address)).to.equal(true);
        });

        it('isSoftRegistered should return false if its in the soft registry alone', async function() {
            await pohSoftContract.connect(userOne).addSubmission('evidence', 'name');
            expect(await proofOfHumanityLevelContract.isSoftRegistered(userOne.address)).to.equal(false);
        });
    })
